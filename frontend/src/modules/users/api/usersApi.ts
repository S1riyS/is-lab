...
export const usersApi = baseApi.injectEndpoints({
    endpoints: (build) => ({
        listUsers: build.query<Page<UserDto>, { page?: number; size?: number; search?: string } | void>({
            query: (params) => ({ url: '/users', params: { page: params?.page ?? 0, size: params?.size ?? 10, search: params?.search } }),
